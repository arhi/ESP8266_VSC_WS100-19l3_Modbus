//
// Originally butchered from ModbusMaster arduino library
//
...
void ModBusRTU::begin(uint8_t slave, Stream &serial)
{
  _u8MBSlave = slave;
  _serial = &serial;
...
void ModBusRTU::beginTransmission(uint16_t u16Address)
{
...
void ModBusRTU::setSlaveID(uint8_t u8Id){
  _u8MBSlave = u8Id;
}

// eliminate this function in favor of using existing MB request functions
uint8_t ModBusRTU::requestFrom(uint16_t address, uint16_t quantity)
{
  uint8_t read = 0;
...
void ModBusRTU::sendBit(bool data)
{
...
void ModBusRTU::send(uint16_t data)
{
...
void ModBusRTU::send(uint32_t data)
{
  send(lowWord(data));
  send(highWord(data));
}
...
void ModBusRTU::send(uint8_t data)
{
  send(word(data));
}


uint8_t ModBusRTU::available(void)
{
  return _u8ResponseBufferLength - _u8ResponseBufferIndex;
}


uint16_t ModBusRTU::receive(void)
{
...
void ModBusRTU::snprintResult(char * buffer, size_t maxlength, uint8_t errorcode ){
    if (errorcode == ku8MBSuccess){
      snprintf(buffer, maxlength, "Success");
    } else if (errorcode == ku8MBResponseTimedOut) {
      snprintf(buffer, maxlength, "Timeout");
    } else if (errorcode == ku8MBIllegalFunction) {
      snprintf(buffer, maxlength, "Illegal Function");
    } else if (errorcode == ku8MBIllegalDataAddress) {
      snprintf(buffer, maxlength, "Illegal Data Address");
    } else if (errorcode == ku8MBIllegalDataValue) {
      snprintf(buffer, maxlength, "Illegal Data Value");
    } else if (errorcode == ku8MBSlaveDeviceFailure) {
      snprintf(buffer, maxlength, "Slave Device Failure");
    } else if (errorcode == ku8MBInvalidSlaveID) {
      snprintf(buffer, maxlength, "Invalid Slave ID");
    } else if (errorcode == ku8MBInvalidFunction) {
      snprintf(buffer, maxlength, "Invalid Function");
    } else if (errorcode == ku8MBInvalidCRC) {
      snprintf(buffer, maxlength, "CRC Error");
    } else {
      snprintf(buffer, maxlength, "Unknown Error %u", errorcode);
    }
}
...
buffer that is used by ModBusRTU. Use of i2c/TWI, 1-Wire, other
serial ports, etc. is permitted within callback function.

@see ModBusRTU::ModBusRTUTransaction()
*/
void ModBusRTU::idle(void (*idle)())
{
  _idle = idle;
}
...
@see ModBusRTU::ModBusRTUTransaction()
@see ModBusRTU::postTransmission()
*/
void ModBusRTU::preTransmission(void (*preTransmission)())
{
...
@see ModBusRTU::ModBusRTUTransaction()
@see ModBusRTU::preTransmission()
*/
void ModBusRTU::postTransmission(void (*postTransmission)())
{
...
@see ModBusRTU::clearResponseBuffer()
@param u8Index index of response buffer array (0x00..0x3F)
@return value in position u8Index of response buffer (0x0000..0xFFFF)
@ingroup buffer
*/
uint16_t ModBusRTU::getResponseBuffer(uint8_t u8Index)
{
...
@see ModBusRTU::getResponseBuffer(uint8_t u8Index)
@ingroup buffer
*/
void ModBusRTU::clearResponseBuffer()
{
  uint8_t i;
...
@see ModBusRTU::clearTransmitBuffer()
@param u8Index index of transmit buffer array (0x00..0x3F)
@param u16Value value to place in position u8Index of transmit buffer (0x0000..0xFFFF)
@return 0 on success; exception number on failure
@ingroup buffer
*/
uint8_t ModBusRTU::setTransmitBuffer(uint8_t u8Index, uint16_t u16Value)
{
  if (u8Index < ku8MaxBufferSize)
  {
...
@see ModBusRTU::setTransmitBuffer(uint8_t u8Index, uint16_t u16Value)
@ingroup buffer
*/
void ModBusRTU::clearTransmitBuffer()
{
  uint8_t i;
...
@ingroup discrete
*/
uint8_t ModBusRTU::readCoils(uint16_t u16ReadAddress, uint16_t u16BitQty)
{
  _u16ReadAddress = u16ReadAddress;
  _u16ReadQty = u16BitQty;
  return ModBusRTUTransaction(ku8MBReadCoils);
}
...
@ingroup discrete
*/
uint8_t ModBusRTU::readDiscreteInputs(uint16_t u16ReadAddress,
  uint16_t u16BitQty)
{
  _u16ReadAddress = u16ReadAddress;
  _u16ReadQty = u16BitQty;
  return ModBusRTUTransaction(ku8MBReadDiscreteInputs);
}
...
@ingroup register
*/
uint8_t ModBusRTU::readHoldingRegisters(uint16_t u16ReadAddress,
  uint16_t u16ReadQty)
{
  _u16ReadAddress = u16ReadAddress;
  _u16ReadQty = u16ReadQty;
  return ModBusRTUTransaction(ku8MBReadHoldingRegisters);
}
...
@ingroup register
*/
uint8_t ModBusRTU::readInputRegisters(uint16_t u16ReadAddress, uint8_t u16ReadQty){
  _u16ReadAddress = u16ReadAddress;
  _u16ReadQty = u16ReadQty;
  return ModBusRTUTransaction(ku8MBReadInputRegisters);
}
...
@ingroup discrete
*/
uint8_t ModBusRTU::writeSingleCoil(uint16_t u16WriteAddress, uint8_t u8State)
{
  _u16WriteAddress = u16WriteAddress;
  _u16WriteQty = (u8State ? 0xFF00 : 0x0000);
  return ModBusRTUTransaction(ku8MBWriteSingleCoil);
}
...
@ingroup register
*/
uint8_t ModBusRTU::writeSingleRegister(uint16_t u16WriteAddress,
  uint16_t u16WriteValue)
{
  _u16WriteAddress = u16WriteAddress;
  _u16WriteQty = 0;
  _u16TransmitBuffer[0] = u16WriteValue;
  return ModBusRTUTransaction(ku8MBWriteSingleRegister);
}
...
@ingroup discrete
*/
uint8_t ModBusRTU::writeMultipleCoils(uint16_t u16WriteAddress,
  uint16_t u16BitQty)
{
  _u16WriteAddress = u16WriteAddress;
  _u16WriteQty = u16BitQty;
  return ModBusRTUTransaction(ku8MBWriteMultipleCoils);
}
uint8_t ModBusRTU::writeMultipleCoils()
{
...
@ingroup register
*/
uint8_t ModBusRTU::writeMultipleRegisters(uint16_t u16WriteAddress,
  uint16_t u16WriteQty)
{
  _u16WriteAddress = u16WriteAddress;
  _u16WriteQty = u16WriteQty;
  return ModBusRTUTransaction(ku8MBWriteMultipleRegisters);
}

// new version based on Wire.h
uint8_t ModBusRTU::writeMultipleRegisters()
{
...
@ingroup register
*/
uint8_t ModBusRTU::maskWriteRegister(uint16_t u16WriteAddress,
  uint16_t u16AndMask, uint16_t u16OrMask)
{
  _u16WriteAddress = u16WriteAddress;
  _u16TransmitBuffer[0] = u16AndMask;
  _u16TransmitBuffer[1] = u16OrMask;
  return ModBusRTUTransaction(ku8MBMaskWriteRegister);
}
...
@ingroup register
*/
uint8_t ModBusRTU::readWriteMultipleRegisters(uint16_t u16ReadAddress,
  uint16_t u16ReadQty, uint16_t u16WriteAddress, uint16_t u16WriteQty)
{
  ...
  _u16WriteAddress = u16WriteAddress;
  _u16WriteQty = u16WriteQty;
  return ModBusRTUTransaction(ku8MBReadWriteMultipleRegisters);
}
uint8_t ModBusRTU::readWriteMultipleRegisters(uint16_t u16ReadAddress,
  uint16_t u16ReadQty)
{
...
/**
Modbus raw read, supply all data manually

@param u8Slaveid address of the slave (0x01 .. 0xff, master is 0)
@param u8Funct modbus function code ()
@param u16ReadAddress address of the first holding register (0x0000..0xFFFF)
@param u16ReadQty quantity of holding registers to read (1..125, enforced by remote device)
...
@ingroup register
*/
uint8_t ModBusRTU::readRaw(uint8_t u8Slaveid, uint8_t u8Funct, uint16_t u16ReadAddress, uint16_t u16ReadQty)
{
  _u8MBSlave = u8Slaveid;
  _u16ReadAddress = u16ReadAddress;
  _u16ReadQty = u16ReadQty;

  return ModBusRTUTransaction(u8Funct);
}
...
@return 0 on success; exception number on failure
*/
uint8_t ModBusRTU::ModBusRTUTransaction(uint8_t u8MBFunction)
{
